...
import { CognitoClient, CognitoIdentityProvider, OAuth2Props } from './cognito-client';
import { MemorySessionStorage } from './session-storage';
import { newUser, setupCognito, user } from './test-utils';
...
describe('Cognito Client', () => {
  ...
  test('generateOAuthSignInUrl', () => {
    const _test = (cb: (searchParams: URLSearchParams) => void, identityProvider?: CognitoIdentityProvider) => {
